...
tools6502::LockstepConfig make_lockstep_config(std::uint8_t setup_memory_fill)
{
    tools6502::LockstepConfig lockstep_config{};
    lockstep_config.memory = tools6502::MemoryFill{setup_memory_fill};
...
GroupStats run_pulse_group(const interrupt_precision::LockstepScenarioRunnerFactory& runner_factory,
                           const char* group_name,
                           const ArbitrationCase& test_case,
                           bool nmi,
                           SummaryStats& summary,
                           FirstFailure& first_failure,
                           std::uint8_t setup_memory_fill)
{
    const auto lockstep_config = make_lockstep_config(setup_memory_fill);
...
GroupStats run_named_script_group(const interrupt_precision::LockstepScenarioRunnerFactory& runner_factory,
                                 const char* group_name,
                                 const ArbitrationCase& test_case,
...
int interrupt_precision::run_interrupt_arbitration(const SuiteContext& context,
                                                  const LockstepScenarioRunnerFactory& runner_factory)
{
    ...
    auto report_group = [&summary, &detail_log, &context](const char* name,
                                                           const ArbitrationCase& test_case,
    ...
    const auto brk_irq_group = run_pulse_group(
        ...
        first_failure,
        context.setup_memory_fill);
    report_group("BRK + IRQ windows", brk_irq_case, brk_irq_group, 0u);
