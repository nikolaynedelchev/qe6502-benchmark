...
std::unique_ptr<cpu6502_bridge::ICpu> make_qe6502()
{
...
{
    const auto cpu = make_qe6502();
    require(std::string(cpu->get_name()) == "qe6502", "unexpected qe6502 ICpu name");
}
...
void test_reset_nop_loop_bus_lockstep()
{
    ...
    tools6502::LockstepRunner runner(make_qe6502(), make_perfect());

    require(runner.setup_and_run(test), "qe6502 setup_and_run(nop loop) failed");
    require(runner.left().is_opcode_fetch(), "qe6502 not at opcode fetch after setup");
    require(runner.left().bus_address() == test.start_at, "qe6502 not at testcase start after setup");

    const auto result = runner.step_cycles(8u);
    require_lockstep_passed(result, "qe6502 NOP loop did not stay bus-lockstepped");
    require(result.left_trace.size() == 8u, "unexpected qe6502 NOP trace length");
...
void test_read_write_program_bus_lockstep()
{
    ...
    require(runner.setup_and_run(test), "qe6502 setup_and_run(read/write) failed");
    const auto result = runner.step_cycles(16u);
    require_lockstep_passed(result, "qe6502 read/write program did not stay bus-lockstepped");
    require(runner.left().memory()[data_address] == 0x43u, "qe6502 read/write program did not update memory");
