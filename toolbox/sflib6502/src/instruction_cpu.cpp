...
class Sflib6502InstructionCpu final : public benchmark6502::IInstructionCpu {
public:
    Sflib6502InstructionCpu()
    {
        cpu_ = M6502_new(&registers_, memory_, &callbacks_);
        reset();
    }

    ~Sflib6502InstructionCpu() override
    {
        if (cpu_ != nullptr) M6502_delete(cpu_);
    }

    Sflib6502InstructionCpu(const Sflib6502InstructionCpu&) = delete;
    Sflib6502InstructionCpu& operator=(const Sflib6502InstructionCpu&) = delete;

    std::string_view core_name() const override { return "sflib6502"; }
...
std::unique_ptr<benchmark6502::IInstructionCpu> make_sflib6502_nmos_instruction_cpu()
{
    return std::make_unique<Sflib6502InstructionCpu>();
}
