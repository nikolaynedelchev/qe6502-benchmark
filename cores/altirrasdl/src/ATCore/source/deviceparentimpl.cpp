...
void ATDeviceBusSingleChild::Init(IATDeviceParent *parent, uint32 busId, uint32 iid, const char *supportedType, const wchar_t *name, const char *tag) {
	VDASSERT(!mpChildDevice);
...
IATDeviceBus *ATDeviceParentSingleChild::GetDeviceBusById(sint32 id) {
	return id == 0 ? &mBus : nullptr;
}

sint32 ATDeviceParentSingleChild::GetDeviceBusIdByIndex(uint32 index) {
	return index == 0 ? 0 : -1;
}
