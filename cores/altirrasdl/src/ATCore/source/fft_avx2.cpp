...
void ATFFT_DIF_Radix8_AVX2(float *dst0, const float *src0, const uint32 *order0, int N) {
	...
	for(int i=0; i<N/32; ++i) {
		...
		const float *__restrict src3 = &src[*order++];
		...
		a_r0r1r2r3 = _mm256_insertf128_ps(a_r0r1r2r3, _mm_load_ps(src3 +  0), 1);
		a_r4r5r6r7 = _mm256_insertf128_ps(a_r4r5r6r7, _mm_load_ps(src3 +  4), 1);
		a_i0i1i2i3 = _mm256_insertf128_ps(a_i0i1i2i3, _mm_load_ps(src3 +  8), 1);
		a_i4i5i6i7 = _mm256_insertf128_ps(a_i4i5i6i7, _mm_load_ps(src3 + 12), 1);
