...
public:
	...
	self_type	operator+(float s) const				{ return self_type { x+s, y+s }; }
	self_type	operator-(float s) const				{ return self_type { x-s, y-s }; }
	...
	self_type	operator*(float s) const				{ self_type a = {x*s, y*s}; return a; }
	self_type&	operator*=(float s)						{ x*=s; y*=s; return *this; }

	self_type	operator/(float s) const				{ const float inv(float(1)/s); self_type a = {x*inv, y*inv}; return a; }
	self_type&	operator/=(float s)						{ const float inv(float(1)/s); x*=inv; y*=inv; return *this; }
