...
struct ATUIDragDropFormatsW32 {
	UINT mContents;			// File contents (paired with file descriptors)
	UINT mDescriptorA;		// File descriptor (narrow char)
	UINT mDescriptorW;		// File descriptor (wide char)
	UINT mShellIdList;		// Shell ID list
	UINT mDropDescription;	// Psuedo-type for setting the drop description on the dragged item
	UINT mDragWindow;		// Psuedo-type for obtaining a handle to the drag item window
};
