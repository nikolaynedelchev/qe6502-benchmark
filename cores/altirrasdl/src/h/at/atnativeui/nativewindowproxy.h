...
public:
	...
	// Return true if this window or a descendant of this window has focus.
	bool HasNestedFocus() const;
	...
	void SetClipSiblings(bool enabled);
	...
	// Raise a window to the top of the complete Z-order, moving the window
	// into the topmost layer.
	void BringToFrontTopmost();

	// Lower a window to the bottom of the Z-order amongst its siblings.
	void SendToBack();
	...
	// Begin capturing mouse input to this window.
	void SetCapture();

	// End capturing mouse input to this window. No effect if not currently
	// capturing.
	void ReleaseCapture();

	// Retrieve the menu attached to a top-level window, if any.
	VDZHMENU GetAttachedMenu() const;
