...
struct ATVMDebugSourceReference {
	uint16 mFileId = 0;
	uint16 mLineId = 0;

	bool operator==(const ATVMDebugSourceReference&) const = default;
};

struct ATVMDebugSourceRange {
	uint32 mCodeOffset = 0;
	ATVMDebugSourceReference mSourceRef;
};
...
struct ATVMFunction {
	...
	vdspan<const ATVMDebugSourceRange> mDebugSourceRanges;

	const ATVMDebugSourceReference *TryGetDebugSourceRefFromCodePos(const uint8 *ip) const;
};
...
public:
	...
	vdfastvector<const char *> mGlobalVariableNames;
...
public:
	...
	const char *mpDebugName = nullptr;
