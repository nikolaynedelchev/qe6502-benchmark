...
void ATDeviceXCmdMountVHD::Invoke(ATDeviceManager& devmgr, IATDevice *dev, int busIndex) {
	...
	const auto devParentBusId = dev->GetParentBusId();
	...
	auto finalize = [&] {
		devmgr.DeserializeDevices(devParent, devParent ? devParent->GetDeviceBusById(devParentBusId) : nullptr, devStr.c_str());
	};
