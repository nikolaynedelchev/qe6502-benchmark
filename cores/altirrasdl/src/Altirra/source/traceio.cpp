...
vdrefptr<IATSerializable> ATSaveTrace(const ATTraceCollection& coll, IATSaveStateSerializer& serializer, const vdfunction<void(int, int)>& progressFn) {
	...
	for(size_t groupIdx = 0, groupCount = coll.GetGroupCount(); groupIdx < groupCount; ++groupIdx) {
		...
		const auto groupType = group->GetType();

		if (groupType != kATTraceGroupType_Normal && groupType != kATTraceGroupType_Log)
			continue;
		...
		if (group->GetType() == kATTraceGroupType_Log)
			savedGroup->mType = ATSavedTraceGroupType::Log;
	...
	for(const auto& grp : root.mGroups) {
		...
		for(const auto& ch : grp->mChannels) {
			...
			if (!traceGroup) {
				...
				if (grp->mType == ATSavedTraceGroupType::Log)
					traceGroup->SetType(kATTraceGroupType_Log);
				else
					traceGroup->SetType(kATTraceGroupType_Normal);
			}
