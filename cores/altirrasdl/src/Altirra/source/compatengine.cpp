...
const ATCompatDBTitle *ATCompatCheckDB(ATCompatDBView& view, vdfastvector<ATCompatKnownTag>& tags) {
	...
	if (IATDiskImage *pImage = g_sim.GetDiskInterface(0).GetDiskImage(); pImage && !pImage->IsDynamic()) {
		markers.clear();
		ATCompatGetMarkersForImage(markers, pImage);
	...
	for(int i=0; i<2; ++i) {
		...
		markers.clear();
		ATCompatGetMarkersForImage(markers, cart->GetImage());
	...
	if (IATCassetteImage *tapeImage = g_sim.GetCassette().GetImage()) {
		markers.clear();
		ATCompatGetMarkersForImage(markers, tapeImage);
	...
	if (programLoader) {
		markers.clear();
		ATCompatGetMarkersForImage(markers, programLoader->GetCurrentImage());
...
void ATCompatGetMarkersForImage(vdfastvector<ATCompatMarker>& markers, IATImage *image) {
