...
std::shared_ptr<ATVideoRecordingSettings> ATUIShowDialogVideoEncoding(VDGUIHandle parent, bool hz50)
{
	...
	auto settings = std::make_shared<ATVideoRecordingSettings>();

	settings->mEncoding = dlg.GetEncoding();
	settings->mFrameRate = dlg.GetFrameRate();
	settings->mAspectRatioMode = dlg.GetAspectRatioMode();
	settings->mResamplingMode = dlg.GetResamplingMode();
	settings->mScalingMode = dlg.GetScalingMode();
	settings->mVideoBitRate = dlg.GetVideoBitRate();
	settings->mAudioBitRate = dlg.GetAudioBitRate();
	settings->mbHalfRate = dlg.GetHalfRate();
	settings->mbEncodeAll = dlg.GetEncodeAllFrames();
