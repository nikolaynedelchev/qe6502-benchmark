...
void ATFDCEmulator::AbortCommand() {
	...
	if (mState == kState_WriteSector_TransferByte) {
		...
		if (mpDiskImage && mpDiskImage->IsValidPhysicalSector(mActivePhysSector) && mpDiskInterface->IsDiskWritable()) {
			try {
		...
		case kState_ReadAddress: {
			...
			if (mpDiskImage && mbMotorRunning && !(mPhysHalfTrack & 1)) {
				...
				for(uint32 sector = 1; sector <= mDiskGeometry.mSectorsPerTrack; ++sector) {
					...
					const auto& psecRange = mpDiskImage->GetPhysicalSectorsForVirtualSector(vsec - 1);
					for(const auto& psec : psecRange) {
						// toss sector if it doesn't match current density
						if (mbMFM != psec.mPhysInfo.mbMFM)
							continue;

						// skip missing sectors and sectors with address CRC errors
						if (!(psec.mPhysInfo.mFDCStatus & 0x10))
							continue;

						float distance = psec.mPhysInfo.mRotPos - posf;
						distance -= floorf(distance);

						if (distance < bestDistance) {
							bestDistance = distance;
							bestPhysSec = psec.mPhysIndex;
							bestRotPos = psec.mPhysInfo.mRotPos;

							// We need uninverted status since we're in the FDC (inversion is
							// handled in the 810 emulation code).
							mActiveSectorStatus = ~psec.mPhysInfo.mFDCStatus;
							bestWeakDataOffset = psec.mPhysInfo.mWeakDataOffset;
							bestSectorSize = psec.mPhysInfo.mPhysicalSize;
							bestVSec = vsec;
							bestVSecCount = (uint32)psecRange.size();
							bestPSecOffset = psec.mPhysOrder;
						}
					}
				}
		...
		case kState_ReadSector: {
			...
			if (mpDiskImage && mRegSector > 0 && mRegSector <= mDiskGeometry.mSectorsPerTrack && mbMotorRunning && !(mPhysHalfTrack & 1)) {
				...
				if (vsi.mNumPhysSectors) {
					...
					float bestRotPos = 0;
					uint32 bestPhysSec = 0;
					uint32 bestPhysOrder = 0;
					int bestWeakDataOffset = -1;
					bool foundValidAddress = false;

					for(const auto& psec : mpDiskImage->GetPhysicalSectorsForVirtualSector(vsec - 1)) {
						...
						if (psec.mPhysInfo.mFDCStatus & 0x18) {
						...
						if (distance < bestDistance) {
							bestDistance = distance;
							bestPhysSec = psec.mPhysIndex;
							bestPhysOrder = psec.mPhysOrder + 1;
							bestRotPos = psec.mPhysInfo.mRotPos;
							...
							mActiveSectorStatus = ~psec.mPhysInfo.mFDCStatus;
							bestWeakDataOffset = psec.mPhysInfo.mWeakDataOffset;
							mTransferLength = std::min<uint32>(psec.mPhysInfo.mPhysicalSize, vdcountof(mTransferBuffer));
						}
					}
...
void ATFDCEmulator::FinalizeWriteTrack() {
	...
	if (mpFnFormatDetect)
		mpFnFormatDetect(mPhysHalfTrack, mbSide2);
	...
	if (!parsedSectors.empty()) {
		...
		for(uint32 i=0; i<sectorsPerTrack; ++i) {
			newVirtSectors[i].mFirstPhysSector = (uint32)newPhysSectors.size();
			...
			if (mbSide2 && mSideMapping != SideMapping::Side2Forward && mSideMapping != SideMapping::Side2ReversedTracks && mSideMapping != SideMapping::Side2Interleaved)
			...
			for(const auto& parsedSector : parsedSectors) {
				...
				psi.mDiskOffset = -1;
				psi.mFDCStatus = 0xFF;
				psi.mImageOffset = parsedSector.mStart;
	...
	IATEditableDiskImage *editableDiskImage = vdpoly_cast<IATEditableDiskImage *>(mpDiskImage);
	if (editableDiskImage)
		editableDiskImage->FormatTrack(GetSelectedVSec(0), sectorsPerTrack, newVirtSectors, (uint32)newPhysSectors.size(), newPhysSectors.data(), mWriteTrackBuffer.data());
...
uint32 ATFDCEmulator::GetSelectedVSec(uint32 sector) const {
	...
	switch(mSideMapping) {
		...
		case SideMapping::Side2Interleaved:
			vtrack = vtrack * 2 + (mbSide2 ? 1 : 0);
			break;
	}
