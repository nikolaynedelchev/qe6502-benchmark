...
void ATDeviceDiskDrive815::EndDiskWrite() {
					...
					if (lastIDAM && i - lastIDAM < 98) {
						...
						for(const SectorRenderInfo& sri : mRenderedSectors) {
								...
								|| (sri.mStartBitPos <= idamBitPos2 && idamBitPos2 < sri.mEndBitPos))
							{
								...
								if (image->IsValidPhysicalSector(sri.mPhysicalSector)) {
	...
	if (doingFormat) {
		...
		for(uint32 secNo = 1; secNo <= maxSecNo; ++secNo) {
			...
			vsi.mFirstPhysSector = (uint32)newPhysSectors.size();
			...
			for(const CapturedSector& cs : capturedSectors) {
				...
				psi.mImageOffset		= (sint32)decodedData.size();
				psi.mDiskOffset			= -1;
				...
				if (!cs.mDataFieldStart) {
					...
					uint8 *sectorData = &decodedData[psi.mImageOffset];
		...
		editableImage->FormatTrack(track * newGeo.mSectorsPerTrack, newGeo.mSectorsPerTrack, newVirtSectors, (uint32)newPhysSectors.size(), newPhysSectors.data(), decodedData.data());
