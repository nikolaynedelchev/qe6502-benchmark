...
void ATDiskRotationTracer::RescanTrack() {
	...
	for(uint32 vsec = vsecStart; vsec < vsecEnd; ++vsec) {
		const auto& psecRange = mpDiskImage->GetPhysicalSectorsForVirtualSector(vsec);

		for(const auto& psec : psecRange) {
			SectorEntry& se = mSectors.emplace_back();
			se.mTickOffset = (uint32)VDRoundToInt32(psec.mPhysInfo.mRotPos * (double)mTicksPerRotation) % mTicksPerRotation;
			se.mVSec = vsec;
			se.mPSecIdx = psecRange.size() > 1 ? psec.mPhysOrder : -1;
		}
	}
