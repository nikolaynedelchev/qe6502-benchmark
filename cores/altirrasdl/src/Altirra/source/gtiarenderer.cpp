	...
	do {
		...
		switch(mPRIOR & 0xc0) {
			...
			case 0x80:
				if (mDefectMode != ATGTIADefectMode::None) [[unlikely]] {
					if (mDefectMode == ATGTIADefectMode::Type1)
						RenderMode10DefectType1(x1, x2);
					else if (mDefectMode == ATGTIADefectMode::Type2 && mbDefectType2Active)
						RenderMode10DefectType2(x1, x2);
					else
						RenderMode10(x1, x2);
				} else if (pmgraphics || mixed)
					RenderMode10(x1, x2);
				else
			...
			case 0xC0:
				if (mDefectMode != ATGTIADefectMode::None) [[unlikely]] {
					if (mDefectMode == ATGTIADefectMode::Type1)
						RenderMode11DefectType1(x1, x2);
					else if (mDefectMode == ATGTIADefectMode::Type2 && mbDefectType2Active)
						RenderMode11DefectType2(x1, x2);
					else
						RenderMode11(x1, x2);
				} else if (pmgraphics || mixed)
					RenderMode11(x1, x2);
...
void ATGTIARenderer::RenderMode9DefectType1(int x1, int x2) {
	...
	if (mbSECAMMode) {
		while(w--) {
			...
			// With defect type I, the first pixel pair of each 4-bit pixel bleeds
			// into the second pair due to errant latching.
			uint8 l1 = lumasrc[0];
		...
		while(w--) {
			...
			const uint8 c4 = pri0 | (l1 & kPlayerMaskLookup[code0 >> 4]);
...
void ATGTIARenderer::RenderMode9DefectType2(int x1, int x2) {
	static const uint8 kPlayerMaskLookup[16]={0xff};
	static const uint8 kPlayerMaskLookupSECAM[16]={0xfe};
	...
	if (mbSECAMMode) {
		while(w--) {
			...
			// Type-II defect causes the GTIA pixel flip-flop to be set
			// incorrectly when enabling GTIA modes mid-line, causing reversed
			// pairing of bit pairs into 4-bit pixels.
...
const uint8 ATGTIARenderer::kMode10Lookup[16]={
	P0,
	P1,
	P2,
	P3,
...
const uint8 ATGTIARenderer::kMode10PFMask[16]={
	0xF0, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF,
...
void ATGTIARenderer::RenderMode10DefectType1(int x1, int x2) {
	const uint8 *__restrict colorTable = mpColorTable;
	const uint8 *__restrict priTable = mpPriTable;
	...
	while(w--) {
		...
		uint8 c4 = kMode10Lookup[l1] & kMode10PFMask[*src & 15];
...
void ATGTIARenderer::RenderMode10DefectType2(int x1, int x2) {
	const uint8 *__restrict colorTable = mpColorTable;
	const uint8 *__restrict priTable = mpPriTable;
	...
	int w = x2 - x1;
	while(w--) {
		const uint8 *__restrict lumasrc = &mpAnticBuffer[(x1++ - 2) | 1];
...
void ATGTIARenderer::RenderMode10(int x1, int x2) {
	...
	while(w2--) {
		...
		uint8 c4 = kMode10Lookup[l1] & kMode10PFMask[src[1] & 15];
...
const uint8 ATGTIARenderer::kMode11Lookup[16][2][2]={
	{{0xff,0xff},{0xff,0xf0}},
	{{0x00,0xff},{0x00,0xff}},
	{{0x00,0xff},{0x00,0xff}},
	{{0x00,0xff},{0x00,0xff}},
...
void ATGTIARenderer::RenderMode11DefectType1(int x1, int x2) {
	const uint8 *__restrict colorTable = mpColorTable;
	const uint8 *__restrict priTable = mpPriTable;
...
void ATGTIARenderer::RenderMode11DefectType2(int x1, int x2) {
	const uint8 *__restrict colorTable = mpColorTable;
	const uint8 *__restrict priTable = mpPriTable;
