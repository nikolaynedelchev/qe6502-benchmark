...
void ATRapidusDevice::Init() {
	...
	mPIAOutput = mpPIA->AllocOutput(
		...
		this,
		IATDevicePIA::kMask_PB0 | IATDevicePIA::kMask_PB1 | IATDevicePIA::kMask_PB4 | IATDevicePIA::kMask_PB7
	);
...
void ATRapidusDevice::DumpStatus(ATConsoleOutput& output) {
	...
	output("$FF0081 Cm. memory CR: $%02X (%s, %s, %s, %s)"
		, mCMCR
		, (mCMCR & 0x40) ? "fastwrite3" : "nofastwrite3"
		, (mCMCR & 0x20) ? "wrap64K" : "nowrap64K"
		, (mCMCR & 0x02) ? "anticext" : "noanticext"
		, (mCMCR & 0x01) ? "cpuext" : "nocpuext"
	);
