...
public:
	...
	bool IsElevationIssue() const {
		return mbElevationIssue;
	}
...
protected:
	...
	HWND mhwndFound = nullptr;
	bool mbElevationIssue = false;
};

ATNotifyOtherInstanceResult ATNotifyOtherInstance(VDCommandLine& cmdLine) {
	ATFindOtherInstanceHelper helper;
	HWND hwndOther = helper.Run();

	if (!hwndOther) {
		if (helper.IsElevationIssue())
			return ATNotifyOtherInstanceResult::OtherInstanceElevated;
		else
